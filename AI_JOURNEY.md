...
The **second prompt** focused on functionality. We initially treated seller input too binarily, but testing that assumption against realistic cases showed that identifiable but incomplete products still deserved useful suggestions. We then brainstormed possible behaviours and features, compared the value they would actually provide to the seller, and separated the essential ones from useful but lower-priority ideas.

The **third prompt** focused on architecture. Once the user experience and functionality were clear, we initially explored a more complex approach to handling model responses, but simplified it after evaluating the trade-offs. We then discussed the minimum technical structure, technology choices, and AI integration needed for the current scope, eventually choosing Groq over the other model options considered.
